...
def plot_group_velocities_fourier(atoms, N=N_FOURIER):
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('group_velocities_fourier_custom_graphene.png')
    plt.show()
...
#         if omega_min < 1e-12:
#             omega_min = 1e-12
#         omega_values = np.linspace(omega_min, omega_max, num_q)
#         delta_omega = (omega_max - omega_min) / (num_q - 1)
#         integrand_values = np.array([
#             integrand(omega, group_velocity, phase_velocity, label)
#             for omega in omega_values
#         ])
#         branch_integral = np.sum(integrand_values) * delta_omega
#         total_integral += branch_integral
#     therm_cond = coeff * total_integral
#     conversion_factor = 1.60218e3
#     therm_cond_in_si = therm_cond * conversion_factor
#     print("Thermal conductivity in SI:", therm_cond_in_si, "W/(m*K)")
