...
gruneisen_param_dict = {
    'LA': 1.8,
    'TA': 1.6,
    'ZA': -1.2  # negative Gruneisen. Use its absolute value when needed.
}
...
def plot_phonon_dispersion(atoms):
    ...
    fc_matrix, R_vectors = compute_force_constant_matrix(atoms)
    frequencies = []
    for q in q_path:
...
#####################################################################################################
#####################################################################################################
## DOES GROUP AND PHASE VELOCITIES COMPUTED CORRECTLY?
...
def compute_group_and_phase_velocities_fourier(atoms, N=N_FOURIER):
    ...
    q_actual = compute_q_arc_length(q_path)
    v_group_dict = {}
    v_phase_dict = {}
